...
class UserProfileDAO {
  ...
  async removeFollowing(uid: string, userIdToRemove: string): Promise<void> {
    return handleFirestoreError(async () => {
      const followingDocRef = doc(this.USER_PROFILE_COLLECTION, `${uid}/following/${userIdToRemove}`);
  ...
  async removeFollower(uid: string, userIdToRemove: string): Promise<void> {
    return handleFirestoreError(async () => {
      const followerDocRef = doc(this.USER_PROFILE_COLLECTION, `${uid}/followers/${userIdToRemove}`);
  ...
  async addFollower(uid: string, followerId: string): Promise<void> {
    return handleFirestoreError(async () => {
      ...
      await setDoc(followerDocRef, { /* data you want to set for follower, if any */ });
    });
  }
  ...
  async addFollowing(uid: string, followingId: string): Promise<void> {
    return handleFirestoreError(async () => {
      ...
      await setDoc(followingDocRef, { /* data you want to set for following, if any */ });
    });
  }
}
